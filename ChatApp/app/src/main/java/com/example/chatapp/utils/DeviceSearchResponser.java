...
/**
 * @author wm
 * @Classname DeviceSearchResponser
 * @Description 用于响应局域网设备搜索
 * @Version 1.0.0
 * @Date 2023/3/6 16:35
 * @Created by wm
 */
public class DeviceSearchResponser {
    ...
    public static void open() {
        ChatAppLog.debug("open response");
    ...
    private static class SearchRespThread extends Thread {
        ...
        public void run() {
            try {
                //指定接收数据包的端口
                socket = new DatagramSocket(Constant.DEVICE_SEARCH_PORT);
        ...
        private byte[] packSearchRespData() {
            ...
            data[offset++] = Constant.PACKET_TYPE_SEARCH_DEVICE_RSP;
        ...
        private boolean verifySearchData(DatagramPacket pack) {
            ...
            int sendSeq;
            if (data[offset++] != '$' || data[offset++] != Constant.PACKET_TYPE_SEARCH_DEVICE_REQ) {
                return false;
            }
            ...
            if (sendSeq < 1 || sendSeq > Constant.SEARCH_DEVICE_TIMES) {
                return false;
            }
